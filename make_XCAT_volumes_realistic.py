...
def run_style_transfer(cnn, normalization_mean, normalization_std, content_img, style_img, input_img, num_steps=300, style_weight=1000, content_weight=1):
...
def load(filename, pmin, pmax, nmin, nmax, main_ax):
    ...
    # roll volume axis as queried (to slice volume along x,y or z)
...
def NST_Volume(Volume, style_img, main_ax, init="content"):
    ...
    assert sx == sy  and sy == sz, "can only translate cubic volumes"
    ...
    for i in range(sx):
        ...
        content_img = totensor(Volume[i, ...]).unsqueeze(0).unsqueeze(0).to(device, torch.float)
        if init == "content":
        ...
        Volume[i, ...] = output.cpu().detach().numpy().squeeze()
...
if __name__ == "main":
    vol_ids = config.split(",")
    # main function to transfer a volume
    def main(vol_id):
        # 1. load and preprocess the XCAT volume
        Volume = load(os.path.join(config.dataroot, vol_id+"_1_CT.nii.gz"))
        # 2. load and preprocess the style CT image
        style_img_numpy = Image.open("./LIDC-IDRI-0139_70.jpg").convert('L')
        style_img = totensor(style_img_numpy).unsqueeze(0).to(device, torch.float)
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = {vol_id: executor.submit(main, vol_id) for vol_id in vol_ids}
    TransferredVolumes = {vol_id: f.result() for vol_id, f in futures.items()}
